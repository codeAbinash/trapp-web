...
export function ClickTextLink({ text, to }: { text: string; to: string }) {
  ...
  return (
    <p className='tap95 bottom-link cursor-pointer rounded-lg px-2 py-1 font-[450] text-color active:bg-color/20'>
      <span
        onClick={transitions(() =>
          navigate(to, {
            replace: true,
...
export function ClickText({ text, onClick = blank_fn }: { text: string; onClick?: any }) {
  return (
    <p className='tap95 cursor-pointer rounded-lg px-2 py-1 font-[450] text-color active:bg-color/20' onClick={onClick}>
      {text}
    </p>
  )
}
